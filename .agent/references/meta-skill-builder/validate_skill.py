...
def validate_skill(skill_name):
    # Assuming the script runs from project root or .agent/references/meta-skill-builder/
    ...
    reference_dir = os.path.join(base_dir, ".agent", "references", skill_name)
    skill_md_path = os.path.join(reference_dir, "SKILL.md")
    ...
    if not os.path.exists(reference_dir):
        errors.append(f"[Structure] Reference directory missing: {reference_dir}")
    ...
    if os.path.exists(skill_md_path):
        with open(skill_md_path, 'r', encoding='utf-8') as f:
            ...
            required_headers = ["Core Principles", "Quality Standards", "Checklist"]
            for header in required_headers:
                if header not in content and header.upper() not in content:
                    errors.append(f"[Content] SKILL.md missing required section: '{header}'")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python .agent/references/meta-skill-builder/validate_skill.py <skill-name>")
        sys.exit(1)

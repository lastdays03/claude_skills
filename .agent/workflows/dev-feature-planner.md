---
description: .agent/references/dev-feature-planner/SKILL.md의 모든 표준(문서화, 리스크 평가, 롤백 전략, 엄격한 TDD)을 준수하는 고급 기능 구현 워크플로우입니다.
---

# Expert Feature Planner Workflow

이 워크플로우는 `.agent/references/dev-feature-planner/SKILL.md`의 "Feature Planner" 표준을 완벽하게 구현하도록 설계되었습니다. 단순한 구현이 아닌, 엔터프라이즈급 안정성과 문서화를 지향합니다.

### 1단계: 심층 분석 및 문맥 파악 (Deep Assessment)
1.  **Context Loading**: `.agent/references/dev-feature-planner/SKILL.md`와 `.agent/references/dev-feature-planner/plan-template.md`를 읽어 '품질 기준'을 재확인합니다.
2.  **Codebase Analysis**:
...
2.  **Drafting**: `.agent/references/dev-feature-planner/plan-template.md`의 **모든 섹션**을 활용하여 `docs/plans/PLAN_[기능명].md` 파일을 작성합니다.
